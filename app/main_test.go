...
func TestMainLoadAllowlistsSuccess(t *testing.T) {
	name := writeTempFile(t, `- integration: foo
  callers:
    - id: caller
      rules:
        - path: /test`)
	...
	entries, err := loadAllowlists(name)
	if err != nil {
		t.Fatalf("loadAllowlists returned error: %v", err)
	}
	if len(entries) != 1 {
		t.Fatalf("expected 1 entry, got %d", len(entries))
	}
	if entries[0].Integration != "foo" {
		t.Fatalf("unexpected integration %q", entries[0].Integration)
	}
	if len(entries[0].Callers) != 1 || len(entries[0].Callers[0].Rules) != 1 {
		t.Fatalf("unexpected callers: %#v", entries[0].Callers)
	}
	if entries[0].Callers[0].Rules[0].Path != "/test" {
		t.Fatalf("unexpected rule path %q", entries[0].Callers[0].Rules[0].Path)
	}
}

func TestMainLoadAllowlistsEmptyFile(t *testing.T) {
...
func TestMainLoadAllowlistsUnknownField(t *testing.T) {
	name := writeTempFile(t, `- integration: foo
  bogus: value`)
	t.Cleanup(func() { os.Remove(name) })
...
func TestMainLoadDenylistsSuccess(t *testing.T) {
	name := writeTempFile(t, `- integration: foo
  callers:
    - id: blocked
      rules:
	...
	entries, err := loadDenylists(name)
	if err != nil {
		t.Fatalf("loadDenylists returned error: %v", err)
	}
	if len(entries) != 1 {
		t.Fatalf("expected 1 entry, got %d", len(entries))
	}
	...
	if entries[0].Callers[0].Rules[0].Path != "/deny" {
		t.Fatalf("unexpected rule path %q", entries[0].Callers[0].Rules[0].Path)
	}
}

func TestMainLoadDenylistsUnknownField(t *testing.T) {
	name := writeTempFile(t, `- integration: foo
  extra: true`)
	t.Cleanup(func() { os.Remove(name) })
...
func TestMainLoadConfigSuccess(t *testing.T) {
	name := writeTempFile(t, `integrations:
- name: test
  destination: http://example.com`)
	...
	if cfg.Integrations[0].Name != "test" || cfg.Integrations[0].Destination != "http://example.com" {
		t.Fatalf("unexpected integration: %#v", cfg.Integrations[0])
	}
}

func TestMainLoadConfigInvalid(t *testing.T) {
	name := writeTempFile(t, `integrations:
- name: invalid`)
	t.Cleanup(func() { os.Remove(name) })
...
func TestMainLoadConfigEmptyFile(t *testing.T) {
	name := writeTempFile(t, "")
	t.Cleanup(func() { os.Remove(name) })
	...
	if cfg == nil || len(cfg.Integrations) != 0 {
...
func (w *testWatcher) Add(name string) error {
	w.addCh <- name
	return nil
}
...
func TestWatchFilesSendsNotifications(t *testing.T) {
	fw := newTestWatcher()
	oldWatcher := newWatcher
	newWatcher = func() (fileWatcher, error) { return fw, nil }
	t.Cleanup(func() { newWatcher = oldWatcher })

	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)

	out := make(chan struct{}, 1)
	go watchFiles(ctx, []string{"config.yaml", "allowlist.yaml"}, out)
	...
	for len(seen) < 2 {
		...
		case <-time.After(time.Second):
			t.Fatal("timeout waiting for initial watch registration")
		}
	}
	if !seen["config.yaml"] || !seen["allowlist.yaml"] {
		t.Fatalf("unexpected watched files: %#v", seen)
	}

	fw.events <- fsnotify.Event{Name: "config.yaml", Op: fsnotify.Write}

	select {
	case <-out:
	case <-time.After(time.Second):
		t.Fatal("expected notification for write event")
	}

	fw.events <- fsnotify.Event{Name: "config.yaml", Op: fsnotify.Rename}

	select {
	case name := <-fw.addCh:
		if name != "config.yaml" {
			t.Fatalf("expected re-add for config.yaml, got %q", name)
		}
	case <-time.After(time.Second):
		t.Fatal("timeout waiting for re-add after rename")
	}
...
func TestHealthzHandler(t *testing.T) {
	...
	if got := resp.Header.Get("X-Last-Reload"); got != "2023-09-01T00:00:00Z" {
		t.Fatalf("unexpected last reload header %q", got)
	}
	resp.Body.Close()
}
...
func TestMetricsHandlerBasicAuth(t *testing.T) {
	...
	*metricsUser, *metricsPass = "user", "pass"
	t.Cleanup(func() {
		*metricsUser = oldUser
		*metricsPass = oldPass
	})
