...
func TestZendeskCapabilities(t *testing.T) {
	caps := integrationplugins.CapabilitiesFor("zendesk")
	if len(caps) != 3 {
	...
	tests := []struct {
		...
		{"open_ticket", "/api/v2/tickets", "POST"},
		{"update_ticket", "/api/v2/tickets/*", "PUT"},
		{"query_status", "/api/v2/tickets/*", "GET"},
	}
